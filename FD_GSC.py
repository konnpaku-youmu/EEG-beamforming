...
class FD_GSC:
    def __init__(self, rir_info, win_len, fft_len):
        ...
        self.audio_data = np.zeros((self.audio_win_len, self.rir_info.num_mics))
        self.fft_len = fft_len
        self.filtered_data = np.zeros((self.audio_win_len, self.rir_info.num_mics))
        self.W_coeffs = None
    ...
    def step(self):
        self.W_coeffs, self.filtered_data = fd_gsc(self.audio_data, self.rir_info, self.fft_len, self.fft_len, self.fft_len//2, self.W_coeffs, self.init_cnt)
        wavfile.write("./audio_outputs/speech_est_1.wav", self.rir_info.sample_rate, self.filtered_data[0] / np.max(self.filtered_data[0]))
...
def fd_gsc(mic, rir_info, fft_length=1024, stft_length=1024, stft_overlap=512, init_w=None, iter_n=None):
    ...
    W_coeffs = None
    if init_w is None:
        W_coeffs = np.zeros((4, stft_length // 2, num_srcs), dtype=np.complex128)
